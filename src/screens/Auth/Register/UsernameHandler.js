...
const RegisterUsername = ({ navigation }) => {
  return (
    <Formik
      ...
      onSubmit={(values) => {
        navigation.navigate("RegisterPassword", { username: values.username.toLowerCase() });
      }}
    >
      {({ handleChange, handleBlur, handleSubmit, values, errors, touched }) => (
        <View className="flex-1 items-center ">
          <View className=" mt-[10] mb-4 w-4/5">
            <Text className=" text-xl text-white text-center font-bold mb-4">Pick username</Text>
